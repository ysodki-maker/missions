...
export default function Cards() {
  ...
  const handleRecharge = async () => {
    ...
    try {
      await rechargeCard(selectedCard.id, { amount: parseFloat(amount) });
      setAmount("");
      await openHistory(selectedCard);
      await loadCards();
      ...
      if (updatedCard) {
        ...
        const loadCount =
          ...
          loadCountData.nbChargements !== undefined
            ...
            ? loadCountData
            : 0;
        setSelectedCard({ ...updatedCard, loadCount });
      }
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 text-sm">Chargement des cartes...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {cards.length === 0 ? (
            <div className="col-span-full bg-white rounded-lg border border-gray-200 p-12 text-center">
              ...
              <p className="text-sm text-gray-500">
                Cet utilisateur n'a pas encore de carte enregistrée
              </p>
            </div>
          ) : (
            cards.map((card) => (
              ...
              >
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${getCardColor(card.type)}`}></div>
                ...
                <div className="flex items-start justify-between">
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      <div className={`w-2 h-2 rounded-full ${getCardColor(selectedCard.type)}`}></div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-semibold text-gray-900 text-sm">
                              +{(load.amount || 0).toLocaleString("fr-FR", {
                                minimumFractionDigits: 2,
                              })}{" "}
                              MAD
                            </p>
                            <p className="text-xs text-gray-500">
                              {new Date(load.createdAt).toLocaleString("fr-FR", {
                                day: "2-digit",
                                month: "short",
                                year: "numeric",
                                hour: "2-digit",
